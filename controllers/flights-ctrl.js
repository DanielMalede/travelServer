...
const getFlightByNumFlight = (req,res)=>{
    const flightName = flights.find(flight=> flight.flightNum == req.params.flightNum)
    flightName ? res.send(flightName) : res.send('flight name not found')
}

const getAllFlightsFirstClass =(req,res)=>{
    const flightsFirstClass = flights.filter(flights=>flights.firstClass==true)
    flightsFirstClass ? res.send({massage:"first class has found",flightsFirstClass:flightsFirstClass}) : res.send('no first class flights found')
}

const getFlightByTimeDepartureTime = (req,res)=>{
    const flightByTime = flights.filter(flights => flights.departureTime == req.params.departureTime)
